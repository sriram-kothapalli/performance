...
for (const username in userEquipmentMap) {
  const { equipment, simUI } = userEquipmentMap[username];

  test(` ${username} - ${equipment} - ${simUI}`, async ({ page }) => {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      if (!fileExists) {
        fs.writeFileSync('performance_metrics.csv', 'Username,JS_Heap_Size_MB,Date,Time,Equipment', 'utf-8');
      }
      const csvData = `\n${username},${jsHeapSize},${currentTimeIST.split(',')[0]},${currentTimeIST.split(',')[1]},${equipment}`;
